...
export function FAQ({ items }: { items: FaqItem[] }) {
  ...
  return (
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className="overflow-hidden rounded-xl border transition-all" style={{ borderColor: open === i ? "var(--color-primary)" : "oklch(88% 0.01 190)" }}>
          <button
          ...
          <div className={`overflow-hidden transition-all duration-300 ${open === i ? "max-h-96" : "max-h-0"}`}>
            <div className="border-t px-5 py-4 text-sm leading-relaxed opacity-70" style={{ borderColor: "oklch(88% 0.01 190)" }}>
              {item.answer}
            </div>
          </div>
        </div>
